...
let settings = { available_capital: null, daily_risk: null, per_position_risk: null };
...
function newPos() {
  return {
    ...
    mgmt: { fe: false, fsl: false, ft: false, fbe: false, note: "" },
    collapsed: false
  };
}
...
async function loadStorageInfo() {
  try {
    ...
    if (currentView === "day") setSaveState("Ready. Saving JSON to " + savePath, "saved", savePath);
  } catch (err) {
    storageOnline = false;
    if (currentView === "day") setSaveState("Server offline. Nothing is being saved to disk.", "", String(err.message || err));
  }
}
...
async function saveSettings() {
  ...
  setSaveState("Settings saved to " + result.path, "saved", result.path);
  await loadDashboard();
  renderApp();
}
...
async function save(opts) {
  ...
  if (!options.silent) setSaveState("Saving to " + savePath + "...", "", savePath);
  try {
    ...
    dirty = false;
    if (!options.silent) setSaveState("Saved " + stamp + " -> " + savePath, "saved", savePath);
    await loadDashboard();
    return true;
  } catch (err) {
    if (!options.silent) setSaveState("Save failed. " + err.message, "", String(err.message || err));
    return false;
  }
}
...
function autoSave() {
  ...
  dirty = true;
  setSaveState("Unsaved changes", "", savePath || "Pending file save");
...
async function loadDashboard() {
  ...
  if (!storageOnline) {
    ...
    setSaveState("Server offline. Nothing is being saved to disk.", "", "Run trade_plan_server.py");
    return;
  }
  try {
    ...
    setSaveState("Ready. Saving JSON to " + savePath, "saved", savePath);
  } catch (_err) {
    dashboardItems = [];
    latestDashboardDate = todayStr();
    setSaveState("Dashboard load failed. Check trade_plan_server.py.", "", "API unavailable");
  }
}
...
function renderDashboardView() {
  ...
  main.innerHTML = `
    <section class="hero">
      ...
      <p>Start here each day. Pick a date to see open positions carried forward from older entries, plus the setups planned for that day.</p>
    </section>
...
function renderSettingsView() {
  ...
  main.innerHTML = `
    <section class="settings-card">
      ...
      <div class="view-note">These defaults feed the dashboard exposure numbers and new trade cards.</div>
...
function renderDayView() {
  ...
  layout.innerHTML = `<div class="day-list">${listHtml || `<div class="plist-empty">No positions saved for this date yet.</div>`}</div><div class="day-detail" id="detail-pane">${selected ? `<div class="detail-shell">${detailHtml}</div>` : detailHtml}</div>`;
...
function buildCard(p, num) {
  ...
  <div class="pcard st-${p._status}" id="card-${p.id}">
    ...
    <div class="pbody ${p.collapsed ? "hide" : ""}" id="pb-${p.id}">
      ...
      <div class="sec"><div class="sec-title">Trade management review <span class="sec-note">fill at close</span></div><div class="checklist"><div class="chk-row"><input type="checkbox" id="fe-${p.id}" ${p.mgmt.fe ? "checked" : ""} onchange="upd('${p.id}','mgmt.fe',this.checked)"><label for="fe-${p.id}">Followed planned entry - did not chase</label></div><div class="chk-row"><input type="checkbox" id="fsl-${p.id}" ${p.mgmt.fsl ? "checked" : ""} onchange="upd('${p.id}','mgmt.fsl',this.checked)"><label for="fsl-${p.id}">Respected SL - exited without hesitation</label></div><div class="chk-row"><input type="checkbox" id="ft-${p.id}" ${p.mgmt.ft ? "checked" : ""} onchange="upd('${p.id}','mgmt.ft',this.checked)"><label for="ft-${p.id}">Executed trims at or near planned levels</label></div><div class="chk-row"><input type="checkbox" id="fbe-${p.id}" ${p.mgmt.fbe ? "checked" : ""} onchange="upd('${p.id}','mgmt.fbe',this.checked)"><label for="fbe-${p.id}">Moved SL to breakeven after day 2</label></div></div><textarea class="dev-note" placeholder="Deviation or lesson - what happened vs plan? What would you do differently?" oninput="upd('${p.id}','mgmt.note',this.value)">${p.mgmt.note || ""}</textarea></div>
    </div>
  </div>`;
}
