...
REPORTS_HTML_DIR = REPORTS_DIR / "generated_html"
NEO_LIQUID_MOMENTUM_SCRIPT = BASE_DIR / "neo_liquid_momentum_scanner.py"
...
def _parse_iso_date_field(value: str) -> date:
...
def _format_report_cell(cell) -> str:
    value = cell.value
    if value is None:
        return ""
...
def _cell_style_to_css(cell) -> str:
    parts = []
    font = cell.font
    fill = cell.fill
    ...
    if font is not None:
        ...
        if font.sz:
            parts.append(f"font-size:{float(font.sz):.1f}px")
        if font.name:
            parts.append(f"font-family:{font.name}, Segoe UI, Arial, sans-serif")
        color = _hex_from_color(getattr(font, "color", None))
        if color:
            parts.append(f"color:{color}")

    if fill is not None and getattr(fill, "patternType", None) == "solid":
        color = _hex_from_color(getattr(fill, "fgColor", None)) or _hex_from_color(getattr(fill, "start_color", None))
        if color:
            parts.append(f"background:{color}")

    if align is not None:
        if align.horizontal:
            parts.append(f"text-align:{align.horizontal}")
        if align.vertical:
            valign = {"top": "top", "center": "middle", "bottom": "bottom"}.get(align.vertical, align.vertical)
            parts.append(f"vertical-align:{valign}")
        if align.wrap_text:
            parts.append("white-space:normal")
        elif align.horizontal in {"left", "right", "center"}:
    ...
    if border is not None:
        for side_name in ("left", "right", "top", "bottom"):
            ...
            if style:
                color = _hex_from_color(getattr(side, "color", None)) or "#2e3450"
                px = "1px"
                if style in {"medium", "thick", "double"}:
                    px = "2px"
                parts.append(f"border-{side_name}:{px} solid {color}")

    return "; ".join(parts)


def convert_stock_rating_workbook_to_html(xlsx_path: Path, html_path: Path) -> Path:
    ...
    def iter_row_cells(ws):
        ...
        for r in range(1, max_row + 1):
            ...
            for c in range(1, max_col + 1):
                ...
                cells.append((cell, rowspan, colspan))
            yield cells

    def sheet_table_html(ws) -> str:
        rows_html = []
        for row_cells in iter_row_cells(ws):
            row_html = []
            for cell, rowspan, colspan in row_cells:
                value = _format_report_cell(cell)
                ...
                row_html.append(
                    f"<td style=\"{css}\"{attr_str}>{html_escape(value)}</td>"
                )
    ...
    for idx, sheet_name in enumerate(sheet_names):
        ws = wb[sheet_name]
        active = " active" if sheet_name == active_sheet else ""
        safe_name = f"sheet-{idx}"
        tabs_html.append(
            f"<button class=\"report-tab{active}\" type=\"button\" data-sheet=\"{safe_name}\">{html_escape(sheet_name)}</button>"
        )
        panes_html.append(
            f"""
            <section class="report-sheet{active}" id="{safe_name}" data-sheet="{safe_name}">
...
<title>{html_escape(xlsx_path.stem)}</title>
<style>
  :root {{
    --bg:#0e1117;
    --panel:#141923;
  ...
  * {{ box-sizing:border-box; }}
  html, body {{ margin:0; min-height:100%; background:linear-gradient(180deg,#0b0f16,#101522 60%, #0b0f16); color:var(--text); font-family:Segoe UI, system-ui, sans-serif; }}
  body {{ padding:18px; }}
  .report-shell {{ max-width:100%; }}
  .report-topbar {{
    display:flex; flex-wrap:wrap; gap:12px; align-items:center; justify-content:space-between;
    background:rgba(20,25,35,.94); border:1px solid var(--line); border-radius:16px;
    padding:14px 16px; margin-bottom:14px; box-shadow:0 12px 30px rgba(0,0,0,.28);
    position:sticky; top:12px; z-index:50; backdrop-filter:blur(10px);
  }}
  .report-title {{ font-size:1.1rem; font-weight:800; color:var(--accent); }}
  .report-subtitle {{ font-size:12px; color:var(--muted); margin-top:3px; }}
  ...
  .report-actions button.active {{ background:var(--accent); border-color:var(--accent); color:#fff; }}
  .report-tabs {{
    display:flex; flex-wrap:wrap; gap:8px; margin:0 0 14px;
    background:rgba(20,25,35,.84); border:1px solid var(--line); border-radius:14px; padding:10px;
  }}
  .report-tab {{
  ...
  .report-sheet {{
    display:none; background:rgba(20,25,35,.94); border:1px solid var(--line);
    border-radius:16px; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,.22);
    margin-bottom:16px;
  }}
  .report-sheet.active {{ display:block; }}
  .report-sheet-head {{
    display:flex; justify-content:space-between; gap:12px; align-items:end; flex-wrap:wrap;
    padding:14px 16px; border-bottom:1px solid var(--line); background:linear-gradient(180deg, rgba(27,34,48,.9), rgba(20,25,35,.94));
  }}
  .report-sheet-title {{ font-size:1rem; font-weight:800; color:var(--text); }}
  .report-sheet-subtitle, .report-sheet-meta {{ font-size:12px; color:var(--muted); }}
  .report-table-wrap {{ overflow:auto; max-height:calc(100vh - 210px); }}
  table.report-table {{ width:max-content; min-width:100%; border-collapse:collapse; font-size:12px; }}
  .report-table td {{
    padding:6px 9px; border:1px solid rgba(255,255,255,.04); vertical-align:middle;
    white-space:nowrap;
  }}
  .report-table tr:hover td {{ outline:1px solid rgba(79,142,247,.16); }}
  @media (max-width: 900px) {{
    body {{ padding:10px; }}
    .report-topbar {{ top:8px; }}
    .report-sheet-head {{ align-items:start; }}
  }}
</style>
</head>
<body>
  <div class="report-shell">
    <div class="report-topbar">
      <div>
        <div class="report-title">Liquid Momentum HTML Report</div>
        <div class="report-subtitle">{html_escape(xlsx_path.name)} rendered from the generated workbook</div>
      </div>
      <div class="report-actions">
        <a href="/download-generated/{html_escape(xlsx_path.name)}" download>Download XLSX</a>
      </div>
    </div>
...
<script>
  ...
  tabs.forEach((tab) => {{
    tab.addEventListener('click', () => {{
      ...
      sheets.forEach((sheet) => sheet.classList.toggle('active', sheet.dataset.sheet === id));
    }});
  }});
</script>
</body>
...
def run_liquid_momentum_report_job(payload: dict) -> dict:
    if not NEO_LIQUID_MOMENTUM_SCRIPT.exists():
        raise FileNotFoundError(f"neo_liquid_momentum_scanner.py not found at {NEO_LIQUID_MOMENTUM_SCRIPT}")

    as_of_raw = str(payload.get("as_of_date") or payload.get("cutoff_date") or "").strip()
    reset_raw = str(payload.get("reset_date") or "").strip()
    ...
    extended = bool(payload.get("extended"))
    run_fundamentals = bool(payload.get("run_fundamentals"))

    cmd = [
        sys.executable,
        str(NEO_LIQUID_MOMENTUM_SCRIPT),
    ...
    if extended:
        cmd.append("--extended")
    if run_fundamentals:
        cmd.append("--run-fundamentals")
    if debug_symbol:
        cmd.extend(["--debug", debug_symbol])
    ...
    xlsx_path = output_dir / f"LiquidCandidates_{as_of_date.strftime('%d%b%Y').upper()}_Reset{reset_date.strftime('%d%b%Y').upper()}.xlsx"
    if not xlsx_path.exists():
        raise RuntimeError(
            "neo_liquid_momentum_scanner.py finished but the workbook was not created.\n"
            f"stdout:\n{proc.stdout}\n\nstderr:\n{proc.stderr}"
        )

    REPORTS_HTML_DIR.mkdir(parents=True, exist_ok=True)
    html_path = REPORTS_HTML_DIR / f"LiquidCandidates_{as_of_date.strftime('%d%b%Y').upper()}_Reset{reset_date.strftime('%d%b%Y').upper()}.html"
    convert_stock_rating_workbook_to_html(xlsx_path, html_path)

    report_url = f"/report-files/{html_path.name}"
    xlsx_url = f"/download-generated/{xlsx_path.name}"
    latest_url = report_url
...
def find_latest_liquid_momentum_html() -> Path | None:
    if not REPORTS_HTML_DIR.exists():
        return None
    candidates = sorted(
        REPORTS_HTML_DIR.glob("LiquidCandidates_*.html"),
...
MAINTENANCE_JOBS = {
    "bhav_sql_batch": {
        ...
        "needs": ["date"],
        "fields": [
            {"name": "date", "label": "Run date", "type": "date", "required": True},
            {"name": "gm", "label": "GM mode", "type": "checkbox", "default": False},
        ],
    },
    "index_smallcaps": {
        ...
        "fields": [
            {
                ...
                "default": "date",
                "options": [
                    {"value": "date", "label": "Range"},
                    {"value": "single", "label": "Single date"},
                ],
            },
            {"name": "date", "label": "Single date", "type": "date"},
    ...
    "ipo_csv": {
        ...
        "needs": [],
        "fields": [
            {"name": "file", "label": "CSV file", "type": "text", "placeholder": "Optional path to a CSV file"},
        ],
    },
    "chittorgarh_ipo": {
        ...
        "needs": ["year_range"],
        "fields": [
            {"name": "start_year", "label": "Start year", "type": "number", "placeholder": "e.g. 2022"},
            {"name": "end_year", "label": "End year", "type": "number", "placeholder": "e.g. 2026"},
        ],
    },
    "moneycontrol_ipo": {
        ...
        "needs": [],
        "fields": [
            {"name": "table", "label": "Table", "type": "text", "placeholder": "Optional MySQL table name"},
            {"name": "limit", "label": "Limit", "type": "number", "placeholder": "Optional row limit"},
            {"name": "min_year", "label": "Minimum year", "type": "number", "placeholder": "Optional filter year"},
        ],
    },
}


def get_admin_jobs_payload():
    jobs = []
    for key, job in MAINTENANCE_JOBS.items():
        jobs.append({
            "key": key,
            "label": job["label"],
            "description": job["description"],
            "needs": list(job.get("needs", [])),
            "fields": list(job.get("fields", [])),
        })
    return jobs
...
def render_page(page_mode: str = "stocks"):
    return render_template_string(
        HTML_PAGE,
        page_mode=page_mode,
        admin_jobs=get_admin_jobs_payload(),
    )
...
def report_files(filename: str):
    REPORTS_HTML_DIR.mkdir(parents=True, exist_ok=True)
    return send_from_directory(REPORTS_HTML_DIR, filename, as_attachment=False)


@app.route("/download-generated/<path:filename>")
def download_generated_file(filename: str):
    candidate = None
    for folder in (BASE_DIR / "output", REPORTS_DIR):
    ...
    return send_from_directory(candidate, filename, as_attachment=True)


@app.route("/api/reports/liquid-momentum/latest")
@app.route("/api/reports/stock-rating/latest")
def api_reports_liquid_momentum_latest():
    latest = find_latest_liquid_momentum_html()
    if not latest:
        return jsonify({"found": False})
    return jsonify({
        "found": True,
        "filename": latest.name,
        "url": f"/report-files/{latest.name}",
        "modified": safe(datetime.fromtimestamp(latest.stat().st_mtime)),
    })


@app.route("/api/reports/liquid-momentum/run", methods=["POST"])
@app.route("/api/reports/stock-rating/run", methods=["POST"])
def api_reports_liquid_momentum_run():
    try:
        payload = request.get_json(force=True, silent=False) or {}
        result = run_liquid_momentum_report_job(payload)
...
def api_admin_jobs():
    return jsonify({"jobs": get_admin_jobs_payload()})


@app.route("/api/admin/jobs/<job_key>/run", methods=["POST"])
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .page-reports #stockControls { display: none; }
  ...
  .page-reports .search-wrap,
  .page-reports .date-range,
  .page-reports #clearDatesBtn,
  .page-reports #loadBtn,
  .page-reports #metaInfo,
  .page-reports #emptyState,
  .page-reports #spinner,
  .page-reports #contentArea,
  .page-reports #sectorPage,
  .page-reports #screenerPage,
  .page-reports #gmlistPage { display: none !important; }
  ...
  .reports-form .full-width { grid-column: 1 / -1; }
  .reports-form label {
    display:block;
    font-size: 11px;
    color: var(--muted);
  ...
  .reports-form input, .reports-form textarea {
    width:100%;
    background: var(--surface);
    color: var(--text);
    border: 1px solid var(--border);
  ...
  .reports-actions button, .reports-actions a {
    padding: 9px 12px;
    border-radius: 8px;
    border: 1px solid var(--border);
    background: var(--accent);
  ...
  .reports-actions a.secondary, .reports-actions button.secondary {
    background: transparent;
    color: var(--muted);
  }
  .reports-status {
  ...
  .reports-status.ok { color: var(--accent2); }
  .reports-status.err { color: var(--red); }
  ...
  .admin-card {
    background: linear-gradient(180deg, rgba(30,34,48,.96), rgba(20,23,34,.98));
    border: 1px solid var(--border);
    border-radius: 16px;
    box-shadow: var(--shadow);
    padding: 16px;
  ...
  .admin-job-actions button.secondary {
    background: transparent;
    color: var(--text);
    border: 1px solid var(--border);
  }
  ...
  @media (max-width: 1200px) {
    .admin-jobs { grid-template-columns: 1fr; }
  }
  .admin-job {
    background: rgba(0,0,0,.12);
    border: 1px solid var(--border);
  ...
  .admin-job-form .full-width { grid-column: 1 / -1; }
  .admin-field label {
    display:block;
    margin-bottom: 5px;
    color: var(--muted);
  ...
  .admin-job-status.ok { color: var(--accent2); }
  .admin-job-status.err { color: var(--red); }
  .admin-log {
    margin-top: 10px;
    width: 100%;
    min-height: 120px;
  ...
  <section id="reportsPage" style="display:none;">
    <div class="reports-card">
        ...
        <div class="reports-actions">
          ...
          <button id="reportsLatestBtn" type="button" class="secondary">Open Latest</button>
          <a id="reportsDownloadXlsx" href="#" class="secondary" style="display:none;" download>Download XLSX</a>
          <a id="reportsOpenHtml" href="#" target="_blank" rel="noopener" style="display:none;">Open HTML</a>
        </div>
      </div>
      ...
      <div class="reports-grid">
        <div class="reports-panel">
          <div class="reports-form">
            <div>
              <label for="reportCutoffDate">As-of date</label>
              <input id="reportCutoffDate" type="date">
            </div>
            <div>
            ...
            <div>
              ...
              <input id="reportDebugSymbol" type="text" placeholder="Optional symbol for diagnostics" autocomplete="off" spellcheck="false">
            </div>
            <div>
              <label for="reportFullMode">Full mode</label>
              <div class="admin-checkbox">
                <input id="reportFullMode" type="checkbox" checked>
                <label for="reportFullMode" style="margin:0;text-transform:none;letter-spacing:0;font-size:13px;color:var(--text)">Append <code>full</code> to the command</label>
              </div>
            </div>
            <div>
              <label for="reportExtended">Extended universe</label>
              <div class="admin-checkbox">
                <input id="reportExtended" type="checkbox">
                <label for="reportExtended" style="margin:0;text-transform:none;letter-spacing:0;font-size:13px;color:var(--text)">Pass <code>--extended</code></label>
              </div>
            </div>
            <div>
              <label for="reportRunFundamentals">Fundamentals</label>
              <div class="admin-checkbox">
                <input id="reportRunFundamentals" type="checkbox">
                <label for="reportRunFundamentals" style="margin:0;text-transform:none;letter-spacing:0;font-size:13px;color:var(--text)">Pass <code>--run-fundamentals</code></label>
              </div>
            </div>
            <div class="full-width">
              <label for="reportConsole">Run output</label>
              <textarea id="reportConsole" readonly placeholder="Run the report to see logs here."></textarea>
            </div>
          </div>
          <div id="reportsStatus" class="reports-status">Pick an as-of date and reset date, then generate the HTML mirror.</div>
        </div>

        <div class="reports-preview">
          <div class="reports-preview-head">
            <div>
              <div class="section-title" style="margin:0;">HTML Preview</div>
              <div class="reports-note">The preview loads the generated browser report from the server.</div>
            </div>
            <div style="display:flex; gap:10px; align-items:baseline; flex-wrap:wrap;">
              <span class="reports-note" id="reportsActiveFile">No report loaded yet.</span>
            </div>
          </div>
          <iframe id="reportsFrame" class="reports-preview-frame" title="Liquid Momentum HTML Report"></iframe>
        </div>
      </div>
    </div>
  </section>
    ...
    <div class="admin-card">
      <div class="admin-head">
        <div>
          ...
          <div class="admin-note">Run backend maintenance jobs from the browser without leaving Bhav Viewer.</div>
        </div>
        <div class="admin-actions">
          <a href="/reports" class="secondary">Open Reports</a>
          <button id="adminRefreshBtn" type="button" class="secondary">Refresh Jobs</button>
        </div>
      </div>
      ...
      <div id="adminStatus" class="admin-status">Loading admin jobs...</div>
    </div>
  </section>
...
function hasAnySectorValue(row) {
  return Boolean(
    String((row && row.sector1) || '').trim() ||
    String((row && row.sector2) || '').trim() ||
    String((row && row.sector3) || '').trim()
  );
}
...
function closeGmListLivePreview() {
  if (gmlistLivePreviewLoading && gmlistLivePreviewLoading.controller) {
    try { gmlistLivePreviewLoading.controller.abort(); } catch (err) {}
  }
...
function buildPreviewCardFromRows(symbol, rows) {
  ...
  for (const r of ordered) {
    ...
    const candle = {
      ...
      close: r.close,
      volume: r.volume,
      change_pct: (r.diff != null ? r.diff : (r.change_pct != null ? r.change_pct : null)),
    };
    candles.push(candle);
    volumes.push({
      time,
  ...
  return {
    ...
    end_close: latestClose,
      move_pct: ordered.length ? ((ordered[ordered.length - 1].diff != null) ? ordered[ordered.length - 1].diff : null) : null,
...
async function openScreenerPreview(symbol, tabKey = '') {
  ...
  if (screenerPreviewLoading && screenerPreviewLoading.controller) {
    try { screenerPreviewLoading.controller.abort(); } catch (err) {}
  }
...
function highlightSectorShortcut(sector) {
  ...
  sectorShortcuts.querySelectorAll('.sector-shortcut').forEach(btn => {
    const labelNode = btn.querySelector('.sector-shortcut-name');
    const label = ((labelNode && labelNode.textContent) || '').trim().toUpperCase();
