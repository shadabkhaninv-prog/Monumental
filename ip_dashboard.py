...
DAILY_POSITION_RUN_FILE = TRADEBOOK_DIR / ".daily_positions_capture.json"
...
def current_ist(now: Optional[datetime] = None) -> datetime:
    return now.astimezone(IST) if now is not None else datetime.now(IST)


def should_capture_daily_positions(now: Optional[datetime] = None) -> bool:
    now_ist = current_ist(now)
    if now_ist.date().weekday() >= 5:
        return False
    market_close = now_ist.replace(hour=15, minute=30, second=0, microsecond=0)
    return now_ist >= market_close


def load_daily_position_run_state(state_file: Path) -> Dict[str, str]:
...
def save_daily_position_run_state(state_file: Path, payload: Dict[str, str]) -> None:
    state_file.parent.mkdir(parents=True, exist_ok=True)
    state_file.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")


def load_kite_trades_snapshot(token_file: str) -> pd.DataFrame:
    ...
    if df.empty:
        return df
    for col in ["tradingsymbol", "exchange", "transaction_type", "quantity", "average_price", "fill_timestamp", "exchange_timestamp"]:
        if col not in df.columns:
            df[col] = ""
    df["tradingsymbol"] = df["tradingsymbol"].astype(str).str.upper().str.strip()
    df["exchange"] = df["exchange"].astype(str).str.upper().str.strip().replace({"": "NSE"})
    ...
    df["segment"] = "EQ"
    df["series"] = "EQ"
    df["trade_type"] = df["transaction_type"].astype(str).str.lower().str.strip()
    df["auction"] = "false"
    df["isin"] = ""
    ...
    out = df[[
        "tradingsymbol", "isin", "trade_date", "exchange", "segment", "series",
        "trade_type", "auction", "quantity", "average_price", "trade_id",
        "order_id", "order_execution_time",
    ]].copy()
    out = out.rename(columns={"tradingsymbol": "symbol", "average_price": "price"})
    out["quantity"] = out["quantity"].round(6)
    out["price"] = out["price"].round(6)
    out["trade_id"] = out["trade_id"].astype(str).str.strip()
    out = out[out["trade_type"].isin(["buy", "sell"])].copy()
    return out.sort_values(["symbol", "exchange", "trade_id"], kind="stable")


def append_daily_positions_snapshot(token_file: str, tradebook_dir: Path) -> Tuple[bool, str]:
    """
    Capture Kite day/net positions once per market day after 3:30 pm and append
    them into the rolling tradebook CSV as synthetic `position` rows.
    """
    now_ist = current_ist()
    if not should_capture_daily_positions(now_ist):
        return False, "Before market close"

    run_state = load_daily_position_run_state(DAILY_POSITION_RUN_FILE)
    today_key = now_ist.date().isoformat()
    if run_state.get("captured_for") == today_key and run_state.get("captured_kind") == "trades":
        return False, f"Already captured for {today_key}"

    snapshot = load_kite_trades_snapshot(token_file)
    if snapshot.empty:
        run_state.update({
            "captured_kind": "trades",
            "captured_for": today_key,
            "captured_at": now_ist.isoformat(),
            "row_count": "0",
        })
        save_daily_position_run_state(DAILY_POSITION_RUN_FILE, run_state)
        return False, "Kite returned no trades"

    tradebook_dir.mkdir(parents=True, exist_ok=True)
    current_year = now_ist.year
    yearly_candidates = sorted(tradebook_dir.glob(f"tradebook-*-{current_year}.csv"))
    if yearly_candidates:
        target = yearly_candidates[0]
    else:
        yearly_files = sorted(tradebook_dir.glob("tradebook-*.csv"))
        if not yearly_files:
            return False, f"No tradebook CSV found in {tradebook_dir}"
        target = next((p for p in yearly_files if p.stem.endswith(f"-{current_year}")), yearly_files[0])

    existing = pd.read_csv(target)
    if "trade_id" not in existing.columns:
        return False, f"Tradebook file missing trade_id column: {target.name}"
    ...
    run_state.update({
        ...
        "captured_for": today_key,
        "captured_at": now_ist.isoformat(),
        "row_count": str(len(snapshot)),
        "target_file": target.name,
    })
    save_daily_position_run_state(DAILY_POSITION_RUN_FILE, run_state)
    return True, f"Appended {len(snapshot)} positions to {target.name}"
...
_daily_capture_status = None
if should_capture_daily_positions():
    _run_state = load_daily_position_run_state(DAILY_POSITION_RUN_FILE)
    _today_key = current_ist().date().isoformat()
    if _run_state.get("captured_for") != _today_key and not st.session_state.get("_daily_positions_capture_done"):
        try:
            _captured, _daily_capture_status = append_daily_positions_snapshot(str(TOKEN_FILE), TRADEBOOK_DIR)
            st.session_state["_daily_positions_capture_done"] = True
        except Exception as exc:
            _daily_capture_status = f"Daily positions capture failed: {exc}"
            st.session_state["_daily_positions_capture_done"] = True
...
if _daily_capture_status:
    st.toast(_daily_capture_status)
